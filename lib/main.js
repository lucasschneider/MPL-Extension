  ...
  worker.port.on('queryGeocoder', function(addr) { // addr[0] = URIencodedAddress; addr[1] = city; addr[2] = addr elt; addr[3] = secondPass bool
    if (addr[3]) {
      var getCntySub = Request({
        url: "http://geocoding.geo.census.gov/geocoder/geographies/address?street="+addr[0]+"&city="+addr[1]+"&state=wi&benchmark=Public_AR_Census2010&vintage=Census2010_Census2010&layers=Counties,Census Tracts,County+Subdivisions&format=json",
...
function onPrefChange(prefName) {
  ...
  if (prefs.validAddr) kohaMods.push(self.data.url("kohaMods/validateAddresses.js"));
  ...
  if (prefs.cssMods) kohaMods.push(self.data.url("kohaMods/cssMods.js"));
...
if (prefs.updateAccountType) kohaPatronEditMods.push(self.data.url("kohaPatronEditMods/updateAccountType.js"));
...
pageMod.PageMod({
  include: /^https?\:\/\/scls-staff\.kohalibrary\.com\/cgi-bin\/koha\/circ\/returns\.pl.*/,
  attachTo: ["top","frame"],
  contentScriptFile:  self.data.url("kohaMods/returnsHotkeys.js")
})