...
function startListening(worker) {
  ...
  worker.port.on('queryTract', function(addr) { // addr[0] = URIencodedAddress; addr[1] = city
    var getTract = Request({
      url: "http://geocoding.geo.census.gov/geocoder/geographies/address?street="+addr[0]+"&city="+addr[1]+"&state=wi&benchmark=Public_AR_Census2010&vintage=Census2010_Census2010&layers=14,Zip+Code+Tabulation+Areas&format=json",
      overrideMimeType: "application/json; charset=UTF-8",
      onComplete: function (response) {
        var match = null;
        ...
        if (match !== undefined && match !== null) {
          ...
          if (match.geographies !== null) {
            // Push zip
	    addrTract.push(match.geographies[ 'Zip Code Tabulation Areas'][0].ZCTA5);
	    // Push tract
