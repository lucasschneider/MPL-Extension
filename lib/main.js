...
function startListening(worker) {
  worker.port.on('queryCntySub', function(addr) {
    var getCntySub = Request({
      url: "http://geocoding.geo.census.gov/geocoder/geographies/address?street="+addr+"&city=madison&state=wi&benchmark=Public_AR_Census2010&vintage=Census2010_Census2010&layers=24&format=json",
	...
	worker.port.emit("receivedCntySub", cntySub);
      }
    }).get();
  });

  worker.port.on('queryTract', function(cntySubAddr) {
    var getTract = Request({
      url: "http://geocoding.geo.census.gov/geocoder/geographies/address?street="+cntySubAddr[1]+"&city=madison&state=wi&benchmark=Public_AR_Census2010&vintage=Census2010_Census2010&layers=14&format=json",
      overrideMimeType: "application/json; charset=UTF-8",
      onComplete: function (response) {
        var match = response.json.result.addressMatches[0];
	var addrTract = [];
	  ...
	  // Push zip
        ...
        worker.port.emit("receivedTract", [cntySubAddr[0], addrTract]);
      }
    }).get();
  });
}
