...
function startListening(worker) {
  worker.port.on('queryTract', function(addr) {
    var getTract = Request({
      url: "http://geocoding.geo.census.gov/geocoder/geographies/address?street="+addr+"&city=madison&state=wi&benchmark=Public_AR_Census2010&vintage=Census2010_Census2010&layers=14,24&format=json",
	...
	var addrTract = [];
	  ...
	  // Push zip
	    ...
	    // Push whether Town of Madison
	    var cntySub =  (match.geographies[ 'County Subdivisions' ] !== null) ? /madison town/i.test(match.geographies[ 'County Subdivisions' ][0].NAME) : null;
	    addrTract.push(cntySub);
	  }
	}
