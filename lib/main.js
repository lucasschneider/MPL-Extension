...
/*** REFRESH FACT FINDER TAB ***/
if (perfs.refreshFactFinder) {
  tabs.on('ready', function (tab) {
    if (/^https?\:\/\/factfinder2\.census\.gov\/faces\/nav\/jsf\/pages\/searchresults\.xhtml.*/.test(tab.url)) {
      var t = tab;
      var refreshFactFinder = timer.setInterval(function (t) { tab.reload() }, 900000);
      var tmr = timer;
      tab.on("close", function (timer) {
        tmr.clearInterval(refreshFactFinder);
      });
    }
  });
}
